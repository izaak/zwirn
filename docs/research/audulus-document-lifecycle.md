...
Before coordinated access, Zwirn updated documents with a direct `fs::write`. Audulus eventually noticed that the file changed, but normally exposed this only through its save-conflict workflow. Choosing Revert reloaded Zwirn’s changes.
...
These experiments led to Zwirn’s current macOS coordinated-access implementation:

- Coordinate the prepared document output itself, rather than performing a direct write followed by an identical helper rewrite.
...
Experiments with `.forReplacing` and `.forMerging` found no benefit over `[]`; the implementation uses neither.
